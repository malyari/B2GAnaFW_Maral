...
Fmistag = ROOT.TFile("MISTAG_RATE_AntiTagMinMass30_ReqTopMassFat_TagMassFatMinMass_QCD_Pt_170toInf_091715.root")
...
# h_mistag_vs_jetPt_AntiTagMinMass30_ReqTopMassFat_TagMassFatMinMass
...
#^ Plot initialization
h_mttbar = ROOT.TH1D("h_mttbar", ";m_{t#bar{t}} (GeV)", 200, 0, 6000)
h_mttbar_true = ROOT.TH1D("h_mttbar_true", "True m_{t#bar{t}};m_{t#bar{t}} (GeV)", 200, 0, 6000)

h_ptLep = ROOT.TH1D("h_ptLep", "Lepton p_{T};p_{T} (GeV)", 100, 0, 1000)
h_etaLep = ROOT.TH1D("h_etaLep", "Lepton #eta;p_{T} (GeV)#eta", 100, 0, ROOT.TMath.TwoPi() )
h_met = ROOT.TH1D("h_met", "Missing p_{T};p_{T} (GeV)", 100, 0, 1000)
h_ptRel = ROOT.TH1D("h_ptRel", "p_{T}^{REL};p_{T}^{REL} (GeV)", 100, 0, 100)
h_dRMin = ROOT.TH1D("h_dRMin", "#Delta R_{MIN};#Delta R_{MIN}", 100, 0, 5.0)
h_2DCut = ROOT.TH2F("h_2DCut", "2D Cut;#Delta R;p_{T}^{REL}", 20, 0, 5.0, 20, 0, 100 )

h_ptAK4 = ROOT.TH1D("h_ptAK4", "AK4 Jet p_{T};p_{T} (GeV)", 300, 0, 3000)
h_etaAK4 = ROOT.TH1D("h_etaAK4", "AK4 Jet #eta;#eta", 120, -6, 6)
h_yAK4 = ROOT.TH1D("h_yAK4", "AK4 Jet Rapidity;y", 120, -6, 6)
h_phiAK4 = ROOT.TH1D("h_phiAK4", "AK4 Jet #phi;#phi (radians)",100,-3.14, 3.14)
h_mAK4 = ROOT.TH1D("h_mAK4", "AK4 Jet Mass;Mass (GeV)", 100, 0, 1000)
h_bdiscAK4 = ROOT.TH1D("h_bdiscAK4", "AK4 b discriminator;b discriminator", 100, 0, 1.0)

h_ptAK8 = ROOT.TH1D("h_ptAK8", "AK8 Jet p_{T};p_{T} (GeV)", 300, 0, 3000)
h_etaAK8 = ROOT.TH1D("h_etaAK8", "AK8 Jet #eta;#eta", 120, -6, 6)
h_yAK8 = ROOT.TH1D("h_yAK8", "AK8 Jet Rapidity;y", 120, -6, 6)
h_phiAK8 = ROOT.TH1D("h_phiAK8", "AK8 Jet #phi;#phi (radians)",100,-3.14, 3.14)
h_mAK8 = ROOT.TH1D("h_mAK8", "AK8 Jet Mass;Mass (GeV)", 100, 0, 1000)
h_mprunedAK8 = ROOT.TH1D("h_mprunedAK8", "AK8 Pruned Jet Mass;Mass (GeV)", 100, 0, 1000)
h_mfilteredAK8 = ROOT.TH1D("h_mfilteredAK8", "AK8 Filtered Jet Mass;Mass (GeV)", 100, 0, 1000)
h_mtrimmedAK8 = ROOT.TH1D("h_mtrimmedAK8", "AK8 Trimmed Jet Mass;Mass (GeV)", 100, 0, 1000)
h_mSDropAK8 = ROOT.TH1D("h_mSDropAK8", "AK8 Soft Drop Jet Mass;Mass (GeV)", 100, 0, 1000)
h_minmassAK8 = ROOT.TH1D("h_minmassAK8", "AK8 CMS Top Tagger Min Mass Paring;m_{min} (GeV)", 100, 0, 1000)
h_nsjAK8 = ROOT.TH1D("h_nsjAK8", "AK8 CMS Top Tagger N_{subjets};N_{subjets}", 5, 0, 5)
h_tau21AK8 = ROOT.TH1D("h_tau21AK8", "AK8 Jet #tau_{2} / #tau_{1};Mass#tau_{21}", 100, 0, 1.0)
h_tau32AK8 = ROOT.TH1D("h_tau32AK8", "AK8 Jet #tau_{3} / #tau_{2};Mass#tau_{32}", 100, 0, 1.0)
...
NeventsCheck =0
Nevents1goodAK8jet =0
Nevents2goodAK8jet =0
...
for ifile in files : #{ Loop over root files
    ...
    for event in events: #{ Loop over events in root files
        ...
        #@ WEIGHT FOR QCD FLAT SAMPLES
        if options.verbose:
            print 'Get flat qcd sample weight'
        ...
        if h_generator.product().hasBinningValues() :
            ...
            if options.deweightFlat:
                FlatWeight = 1/pow(pthat/15.,4.5)
        if options.verbose:
            print 'pthat = '+str(pthat)
            print 'FlatWeight = '+str(FlatWeight)
        ...
        if not Leptonic : 
            ...
            if len( h_jetsAK8Pt.product()) > 0 : #{ make sure AK8 jets exist
                NeventsAK8product +=1
                ...
                for i in range(0,len(AK8Pt)):#{ Loop over AK8 Jets
                    ...
                    if AK8P4Corr.Perp() < options.minAK8Pt or abs(AK8P4Raw.Rapidity()) > options.maxAK8Rapidity :
                        continue
                    ...
                    #$ Cuts for Hadronic channel
                        ...
                        ak8JetsGoodTopSubjetIndex3.append( AK8TopSubjetIndex3[i] )                        
                #} End AK8 Loop
            #} end check that AK8 jets exist
            NeventsCheck +=1

            #make sure there is at least 1 good AK8 jet (needed for semi-leptonic and hadronic selections)
            if len(ak8JetsGood) < 1 :
                if options.verbose :
                    print 'Not enough AK8 jets, skipping'
                continue

            Nevents1goodAK8jet +=1
            if len(ak8JetsGood) > 1 :
                Nevents2goodAK8jet +=1
            ...
            if len(ak8JetsGood) > 1 and Hadronic: #{ check that we have at least 2 AK8 jets for all-hadronic background estimation    
                ...
                if topTag0MassSD and topTag1MassSD:
                    h_mttMass_tagMassSD.Fill( ttMass, FlatWeight )
                if topTag0MassSDTau32 and topTag1MassSDTau32:
                    h_mttMass_tagMassSDTau32.Fill( ttMass, FlatWeight )
                if topTag0MassSDMinMass and topTag1MassSDMinMass:
                    h_mttMass_tagMassSDMinMass.Fill( ttMass, FlatWeight )
                if topTag0MassFat and topTag1MassFat:
                    h_mttMass_tagMassFat.Fill( ttMass, FlatWeight )
                if topTag0MassFatMinMass and topTag1MassFatMinMass:
                    h_mttMass_tagMassFatMinMass.Fill( ttMass, FlatWeight )
                ...
                if x < 0.5 :
                    ...
                    # if topTag0MassSD :
                    #     if options.verbose:
                    #         print '  tagged topTag0MassSD -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[1].Perp())+' tagged = '+str(topTag1MassSD)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSD        .Accumulate(              ttMass, ak8JetsGood[1].Perp(), topTag1MassSD, FlatWeight )
                    #     mttPredDist_modMass_tagMassSD.Accumulate( ttMass_modMass_jet1, ak8JetsGood[1].Perp(), topTag1MassSD, FlatWeight )
                    # if topTag0MassSDTau32 :
                    #     if options.verbose:
                    #         print '  tagged topTag0MassSDTau32 -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[1].Perp())+' tagged = '+str(topTag1MassSDTau32)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSDTau32        .Accumulate(              ttMass, ak8JetsGood[1].Perp(), topTag1MassSDTau32, FlatWeight )
                    #     # mttPredDist_modMass_tagMassSDTau32.Accumulate( ttMass_modMass_jet1, ak8JetsGood[1].Perp(), topTag1MassSDTau32, FlatWeight )
                    # if topTag0MassSDMinMass :
                    #     if options.verbose:
                    #        print '  tagged topTag0MassSDMinMass -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[1].Perp())+' tagged = '+str(topTag1MassSDMinMass)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSDMinMass        .Accumulate(              ttMass, ak8JetsGood[1].Perp(), topTag1MassSDMinMass, FlatWeight )
                    #     # print 'debug1'
                    #     # mttPredDist_modMass_tagMassSDMinMass.Accumulate( ttMass_modMass_jet1, ak8JetsGood[1].Perp(), topTag1MassSDMinMass, FlatWeight )
                    #     # print 'debug2'
                    # if topTag0MassFat :
                    #     # print 'debug3'
                    #     if options.verbose:
                    #         print '  tagged topTag0MassFat -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[1].Perp())+' tagged = '+str(topTag1MassFat)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassFat        .Accumulate(              ttMass, ak8JetsGood[1].Perp(), topTag1MassFat, FlatWeight )
                    #     mttPredDist_modMass_tagMassFat.Accumulate( ttMass_modMass_jet1, ak8JetsGood[1].Perp(), topTag1MassFat, FlatWeight )
                    #     bin   = h_mistag_vs_jetPt_TagMassFat.GetXaxis().FindBin( ak8JetsGood[1].Perp() )
                    #     rate  = h_mistag_vs_jetPt_TagMassFat.GetBinContent(bin);
                    #     h_bkgdEst_tagMassFat .Fill(ttMass, FlatWeight*rate)
                    if topTag0MassFatMinMass :
                        if options.verbose:  
                            print '  tagged topTag0MassFatMinMass -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[1].Perp())+' tagged = '+str(topTag1MassFatMinMass)+ ' FlatWeight '+ str(FlatWeight)
                        mttPredDist_tagMassFatMinMass        .Accumulate(              ttMass, ak8JetsGood[1].Perp(), topTag1MassFatMinMass, FlatWeight )
                        # mttPredDist_modMass_tagMassFatMinMass.Accumulate( ttMass_modMass_jet1, ak8JetsGood[1].Perp(), topTag1MassFatMinMass, FlatWeight )
                        ...
                        h_bkgdEst_tagMassFatMinMass .Fill(ttMass, FlatWeight*rate)
                        h_bkgdEst_modMass_tagMassFatMinMass .Fill(ttMass_modMass_jet1, FlatWeight*rate)
                ...
                if x >= 0.5 :
                    ...
                    # if topTag1MassSD :
                    #     if options.verbose:
                    #         print '  tagged topTag1MassSD -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[0].Perp())+' tagged = '+str(topTag0MassSD)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSD        .Accumulate(              ttMass, ak8JetsGood[0].Perp(), topTag0MassSD, FlatWeight )
                    #     mttPredDist_modMass_tagMassSD.Accumulate( ttMass_modMass_jet0, ak8JetsGood[0].Perp(), topTag0MassSD, FlatWeight )
                    # if topTag1MassSDTau32 :
                    #     if options.verbose:
                    #         print '  tagged topTag1MassSDTau32 -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[0].Perp())+' tagged = '+str(topTag0MassSDTau32)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSDTau32        .Accumulate(              ttMass, ak8JetsGood[0].Perp(), topTag0MassSDTau32, FlatWeight )
                    #     # mttPredDist_modMass_tagMassSDTau32.Accumulate( ttMass_modMass_jet0, ak8JetsGood[0].Perp(), topTag0MassSDTau32, FlatWeight )
                    # if topTag1MassSDMinMass :
                    #     if options.verbose:
                    #         print '  tagged topTag1MassSDMinMass -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[0].Perp())+' tagged = '+str(topTag0MassSDMinMass)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassSDMinMass        .Accumulate(              ttMass, ak8JetsGood[0].Perp(), topTag0MassSDMinMass, FlatWeight )
                    #     # mttPredDist_modMass_tagMassSDMinMass.Accumulate( ttMass_modMass_jet0, ak8JetsGood[0].Perp(), topTag0MassSDMinMass, FlatWeight )
                    # if topTag1MassFat :
                    #     # print 'debug5'
                    #     if options.verbose:
                    #         print '  tagged topTag1MassFat -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[0].Perp())+' tagged = '+str(topTag0MassFat)+ ' FlatWeight '+ str(FlatWeight)
                    #     mttPredDist_tagMassFat        .Accumulate(              ttMass, ak8JetsGood[0].Perp(), topTag0MassFat, FlatWeight )
                    #     mttPredDist_modMass_tagMassFat.Accumulate( ttMass_modMass_jet0, ak8JetsGood[0].Perp(), topTag0MassFat, FlatWeight )
                    #     bin   = h_mistag_vs_jetPt_TagMassFat.GetXaxis().FindBin( ak8JetsGood[0].Perp() )
                    #     rate  = h_mistag_vs_jetPt_TagMassFat.GetBinContent(bin);
                    #     h_bkgdEst_tagMassFat .Fill(ttMass, FlatWeight*rate)
                    if topTag1MassFatMinMass :
                        if options.verbose:
                            print '  tagged topTag1MassFatMinMass -> Accumulate ( ttmass = '+str(ttMass)+' pt = '+str(ak8JetsGood[0].Perp())+' tagged = '+str(topTag0MassFatMinMass)+ ' FlatWeight '+ str(FlatWeight)
                        mttPredDist_tagMassFatMinMass        .Accumulate(              ttMass, ak8JetsGood[0].Perp(), topTag0MassFatMinMass, FlatWeight )
                        # mttPredDist_modMass_tagMassFatMinMass.Accumulate( ttMass_modMass_jet0, ak8JetsGood[0].Perp(), topTag0MassFatMinMass, FlatWeight )
                        # do it again with incorrect error bars
                        bin   = h_mistag_vs_jetPt_TagMassFatMinMass.GetXaxis().FindBin( ak8JetsGood[0].Perp() )
                        rate  = h_mistag_vs_jetPt_TagMassFatMinMass.GetBinContent(bin);
                        h_bkgdEst_tagMassFatMinMass .Fill(ttMass, FlatWeight*rate)
                        h_bkgdEst_modMass_tagMassFatMinMass .Fill(ttMass_modMass_jet0, FlatWeight*rate)
            ...
            #@ AK8 jet loop to fill histograms
            nttags = 0
            tJets = []
            countAK8 =0
        ...
        #@ Semileptonic
        elif SemiLeptonic == True and nttags >= 1 :     #{ Start if SemiLeptonic
        ...
        #@ Hadronic
        elif Hadronic and nttags >= 2 : #{ Start if Hadronic
...
print 'Number of Events with AK8product '+str(NeventsAK8product)
print 'Number of Events with BkgdEstimation '+str(NeventsBkgdEstimation)
print 'NeventsCheck   ' + str(NeventsCheck)
print 'Nevents1goodAK8jet   ' + str(Nevents1goodAK8jet)
print 'Nevents2goodAK8jet   ' + str(Nevents2goodAK8jet)
print '========================================='
...
# mttPredDist_tagMassSD         .GetPredictedHist().Write()
# mttPredDist_tagMassSDTau32    .GetPredictedHist().Write()
# mttPredDist_tagMassSDMinMass  .GetPredictedHist().Write()
# mttPredDist_tagMassFat        .GetPredictedHist().Write()
# mttPredDist_tagMassFatMinMass .GetPredictedHist().Write()

# mttPredDist_tagMassSD         .GetObservedHist().Write()
# mttPredDist_tagMassSDTau32    .GetObservedHist().Write()
# mttPredDist_tagMassSDMinMass  .GetObservedHist().Write()
# mttPredDist_tagMassFat        .GetObservedHist().Write()
# mttPredDist_tagMassFatMinMass .GetObservedHist().Write()

# mttPredDist_tagMassSD         .GetTaggableHist().Write()
# mttPredDist_tagMassSDTau32    .GetTaggableHist().Write()
# mttPredDist_tagMassSDMinMass  .GetTaggableHist().Write()
# mttPredDist_tagMassFat        .GetTaggableHist().Write()
# mttPredDist_tagMassFatMinMass .GetTaggableHist().Write()


# mttPredDist_modMass_tagMassSD         .GetPredictedHist().Write()
# mttPredDist_modMass_tagMassSDTau32    .GetPredictedHist().Write()
# mttPredDist_modMass_tagMassSDMinMass  .GetPredictedHist().Write()
# mttPredDist_modMass_tagMassFat        .GetPredictedHist().Write()
# mttPredDist_modMass_tagMassFatMinMass .GetPredictedHist().Write()

# mttPredDist_modMass_tagMassSD         .GetObservedHist().Write()
# mttPredDist_modMass_tagMassSDTau32    .GetObservedHist().Write()
# mttPredDist_modMass_tagMassSDMinMass  .GetObservedHist().Write()
# mttPredDist_modMass_tagMassFat        .GetObservedHist().Write()
# mttPredDist_modMass_tagMassFatMinMass .GetObservedHist().Write()

# mttPredDist_modMass_tagMassSD         .GetTaggableHist().Write()
# mttPredDist_modMass_tagMassSDTau32    .GetTaggableHist().Write()
# mttPredDist_modMass_tagMassSDMinMass  .GetTaggableHist().Write()
# mttPredDist_modMass_tagMassFat        .GetTaggableHist().Write()
# mttPredDist_modMass_tagMassFatMinMass .GetTaggableHist().Write()
